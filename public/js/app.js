            ...
            .on("click", function (e) {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        showToast('Uploaded Successfully!','Assignment Uploaded Successfully!','success')
                        countAssignments();
                        $(".loading").hide();
                        $(".loading-text").show();
                    },
...
// Function to display assignments based on the current filter
    ...
    const assignmentsHtml = assignments.assignments?.map((assignment, index) => {
...
$(document).ready(function () {
    ...
    if (filterType === "submitted") {
        filteredAssignments = allAssignment.assignments.filter((assignment, index) => {
    ...
    const filteredAssignmentsObj = {
        ...
        status: filteredStatus.filter((_, index) => {
            return (filterType === "submitted" && filteredStatus[index]?.status === 'submitted') ||
                   (filterType === "unsubmitted" && filteredStatus[index]?.status !== 'submitted') ||
                   (filterType === "all"); // Include all statuses if filter is "all"
        })
    };
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.status === "success") {
                    ...
                    showToast('Uploaded Successfully',response.message,'success')
                }
            },
            error: function (xhr) {
                showErrorMessages(xhr.responseJSON.errors);
                ...
                .on("click", ".mark-assignment", function () {
                    ...
                    $.ajax({
                        ...
                        beforeSend: function () {
                            ...
                            $(
                        `input[name="obtainedMarks-${answerId}"]`
                    ).prop('disabled',true).val('marking...')
                            $(
                        `input[name="obtainedMarks-${answerId}"]`
                    ).siblings('.alert-danger').remove()
                        ...
                        success: function (response) {
                            $button
                                ...
                                .attr("disabled", true)
                                .addClass("btn-disabled");
                                showToast('Marked Successfully','Assignment marked successfully','success')
                              $(
                            `input[name="obtainedMarks-${answerId}"]`
                              ).prop('disabled', true).val('marked successfully')
                              $(
                            `input[name="obtainedMarks-${answerId}"]`
                              ).siblings('.alert-danger').remove()
