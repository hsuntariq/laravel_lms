...
$(document).ready(function () {
    ...
    $(document).on("click", ".batch-pagination .pagination a", function (e) {
        e.preventDefault();
...
$(document).ready(function () {
    ...
    $(document).on("change", ".courses-select", function () {
        ...
        $.ajax({
            ...
            beforeSend: function () {
                  $('.batch-select').html('<option disabled selected>Loading batches...</option>')
            },
            success: function (response) {

                let batchOptions = '<option disabled selected>Select Batch</option>';
                ...
                $(".total-students").html(`Total Students(${response.options.data.length})`);
            },
            complete: function () {
    ...
    $(document).on("click", ".delete-student", function (e) {
        ...
        if (confirm("Are you sure you want to delete this student?")) {
            $.ajax({
                ...
                $(`.delete-student[data-id="${studentId}"]`).attr('disabled', true).text('Deleting...');
            },
                success: function (response) {
...
$(document).on("submit", ".student-edit-form", function (e) {
    ...
    $.ajax({
        ...
        complete: function () {
            $(".update-student-btn").attr("disabled", false).text("Update Student");
        },
    });
});
...
function deleteStudent(studentId) {
    $.ajax({
            ...
            error: function (xhr) {
                showFlashMessage('Error deleting student. Please try again.', 'danger');
            },
            complete: function () {
                // Reset the button state
                $(`.delete-student[data-id="${studentId}"]`).attr('disabled', false).text('Delete');
            }
        });
}
...
$(document).ready(function () {
    ...
    function fetchCourses(selectedCourseId = null, selectedBatchId = null) {
        $.ajax({
            ...
            success: function (response) {
                let coursesHtml = '<option disabled selected>Select Course</option>';
                response.forEach(course => {
                    coursesHtml += `<option value="${course.id}" ${selectedCourseId === course.id ? 'selected' : ''}>${course.course_name}</option>`;
                });
            ...
            error: function () {
                $("#edit_course").html('<option>Error loading courses</option>');
            }
        });
    }
    ...
    function fetchBatches(courseId, selectedBatchId = null) {
        $.ajax({
            ...
            success: function (response) {
                console.log(response)
                let batchesHtml = '<option disabled selected>Loading batches...</option>';
                response.batches.forEach(batch => {
                    batchesHtml += `<option value="${batch.id}" ${selectedBatchId === batch.id ? 'selected' : ''}>${batch.batch_no}</option>`;
                });
